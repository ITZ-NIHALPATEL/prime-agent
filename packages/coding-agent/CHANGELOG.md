...
- Added an async `bash()` function to the IPython kernel so `await bash("...")` runs shell commands via `asyncio.create_subprocess_exec` without blocking the kernel event loop.
